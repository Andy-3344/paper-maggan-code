...
class MAGCN(nn.Module):
    def __init__(self, args):
        ...
        self.cnn_x = nn.Conv2d(in_channels=self.args.lncRNA_view * self.args.gcn_layers,
                               out_channels=self.args.out_channels,
                               kernel_size=(self.args.fl, 1),
                               stride=1,
                               bias=True)
        self.cnn_y = nn.Conv2d(in_channels=self.args.disease_view * self.args.gcn_layers,
                               out_channels=self.args.out_channels,
    ...
    def forward(self, data):
        ...
        # x_l_lfs1 = torch.relu(self.gcn_x1_lfs(x_l.cuda(), data['lfs']['edges'].cuda(), data['lfs']['data_matrix'][
        #     data['lfs']['edges'][0], data['lfs']['edges'][1]].cuda()))
        # x_l_lfs2 = torch.relu(self.gcn_x2_lfs(x_l_lfs1, data['lfs']['edges'].cuda(), data['lfs']['data_matrix'][
        #     data['lfs']['edges'][0], data['lfs']['edges'][1]].cuda()))

        x_l_lgs1 = torch.relu(self.gcn_x1_lgs(x_l.cuda(), data['lgs']['edges'].cuda(), data['lgs']['data_matrix'][
            data['lgs']['edges'][0], data['lgs']['edges'][1]].cuda()))
        x_l_lgs2 = torch.relu(self.gcn_x2_lgs(x_l_lgs1, data['lgs']['edges'].cuda(), data['lgs']['data_matrix'][
            data['lgs']['edges'][0], data['lgs']['edges'][1]].cuda()))

        x_l_lss1 = torch.relu(self.gcn_x1_lss(x_l.cuda(), data['lss']['edges'].cuda(), data['lss']['data_matrix'][
            data['lss']['edges'][0], data['lss']['edges'][1]].cuda()))
        x_l_lss2 = torch.relu(self.gcn_x2_lss(x_l_lss1, data['lss']['edges'].cuda(), data['lss']['data_matrix'][
            data['lss']['edges'][0], data['lss']['edges'][1]].cuda()))

        # y_d_dss1 = torch.relu(self.gcn_y1_dss(x_d.cuda(), data['dss']['edges'].cuda(), data['dss']['data_matrix'][
        #     data['dss']['edges'][0], data['dss']['edges'][1]].cuda()))
        # y_d_dss2 = torch.relu(self.gcn_y2_dss(y_d_dss1, data['dss']['edges'].cuda(), data['dss']['data_matrix'][
        #     data['dss']['edges'][0], data['dss']['edges'][1]].cuda()))

        y_d_dgs1 = torch.relu(self.gcn_y1_dgs(x_d.cuda(), data['dgs']['edges'].cuda(), data['dgs']['data_matrix'][
            data['dgs']['edges'][0], data['dgs']['edges'][1]].cuda()))
        y_d_dgs2 = torch.relu(self.gcn_y2_dgs(y_d_dgs1, data['dgs']['edges'].cuda(), data['dgs']['data_matrix'][
            data['dgs']['edges'][0], data['dgs']['edges'][1]].cuda()))

        # XM = torch.cat((x_l_lfs1, x_l_lfs2, x_l_lgs1, x_l_lgs2, x_l_lss1, x_l_lss2), 1).t()
        XM = torch.cat((x_l_lgs1, x_l_lgs2, x_l_lss1, x_l_lss2), 1).t()
        ...
        YD = torch.cat((y_d_dgs1, y_d_dgs2), 1).t()
        ...
        if self.temp == self.args.epoch:
            ...
            save_x = save_x.round(4)
            save_x.to_csv(self.args.dataset_path +'/lncRNAFeature.csv', header=None, index=None)
            savey = y.cpu().detach().numpy()
            save_y = pd.DataFrame(savey)
            save_y = save_y.round(4)
            save_y.to_csv(self.args.dataset_path +'/diseaseFeature.csv', header=None, index=None)
        return y.mm(x.t())
