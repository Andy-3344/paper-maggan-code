...
def eval_epoch(model, data_iter, criterion):
    ...
    with torch.no_grad():
        for (data, target) in data_iter:
            # data_iter, min interval=2, desc=' - Training', leave=False):
            data = Variable(data)
            target = Variable(target)
            if opt.cuda:
                data, target = data.cuda(), target.cuda()
    ...
    assert total_words > 0  # Otherwise Null pointerException
    return math.exp(total_loss / total_words)
...
def main():
    ...
    for total_batch in range(TOTAL_BATCH):
        ...
        for it in range(1):
            samples = generator.sample(64, g_sequence_len)
            # construct the input to the generator, add zeros before samples and delete the last column
            zeros = torch.zeros((64, 1)).type(torch.LongTensor)
            if samples.is_cuda:
                zeros = zeros.cuda()
        ...
        for _ in range(1):
            ...
            for _ in range(2):
                loss = train_epoch(discriminator, dis_data_iter, dis_criterion, dis_optimizer)
        print('Epoch [%d], loss: %f' % (total_batch, loss))
